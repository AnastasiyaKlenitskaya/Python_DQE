...
def generating_of_dictionary_with_random_values(keys_amount=random.randint(3, 26)) -> dict:
...
def generating_list_of_dicts_with_random_values(number_of_dicts_in_list=random.randint(2, 10)) -> list:
    list_of_dicts = []  # initialization of variable to collect generated dictionaries
    while True:  # Loop for filling in list by dicts
        list_of_dicts.append(generating_of_dictionary_with_random_values())     # adding generated dict to the list
        if len(list_of_dicts) >= number_of_dicts_in_list:      # exiting from the loop when needed amount of dicts added
